...
const PROFILE: SourceContract = {
  ...
  fields: [
    ...
    { path: 'business.turnover', what: 'annual turnover band', consumedBy: [], note: 'DEAD. Order-size sanity, budget banding and B2B/B2C judgement all guess without it.' },
    ...
    { path: 'kyb.legal_status', what: 'proprietorship / pvt ltd / …', consumedBy: [], note: 'DEAD. Legal status is a direct read on B2B-ness and the planner is left to infer it from the category.' },
    { path: 'kyb.registration_year', what: 'GST registration year', consumedBy: [], note: 'DEAD' },
    ...
    { path: 'seller_context', what: 'product_sold · fcp_flag · custtype_name — i.e. THIS BUYER IS ALSO A PAID SELLER', consumedBy: [], sinceVersion: 'requirement-brain v8', note: 'THE named incident. Parsed by buyer-brain, then dropped by the node_raw output whitelist so only the opaque custtype_weight survived. v8 re-emitted it — and it STILL has zero consumers, so a buyer who is a paid seller is treated exactly like a cold buyer.' },
...
const CALLS: SourceContract = {
  ...
  fields: [
    ...
    { path: 'buyer.b2b_b2c', what: 'B2B/B2C read for THIS buyer from his own call', consumedBy: [], note: 'DEAD — while category.b2b_b2c (a corpus average) IS passed to the planner. The per-buyer truth loses to the category average.' },
    { path: 'buyer.persona', what: 'persona for THIS buyer from his own call', consumedBy: [], note: 'DEAD — same inversion as b2b_b2c: category.personas is passed, the buyer own persona is not.' },
