...
export const EXTRACT_BUYER_PROFILE_SYSTEM = [
  ...
  "- sourcing_channel = HOW / WHERE the buyer sources — IndiaMART BuyLeads, direct masked-number calls to sellers, and the specific supplier cities / markets engaged — from the PNS call basket + BuyLead search-cities + CSL supplier visits (buyer-side only, never our-outbound). Preferred sources: PNS call basket + BuyLead search-cities + CSL supplier visits (buyer-side only).",
