...
function composeWebOsint(summary: unknown, basis: unknown, meta: unknown, src: string, push: (src: string, tag: string, raw: unknown) => void): boolean {
  ...
  // owner obs-4a (2026-07-13) — the Dinesh "indomaret.co.id" fix: TAGGING flagged fields was not enough (the LLM + card
  // still read the wrong entity). When the web result is NOT tied to THIS buyer — match_confidence none/low, OR every
  // identity-defining field present is namesake-flagged — SUPPRESS the whole web summary block: withhold the field VALUES
  // entirely and emit ONE honest line. A namesake with zero shared hard-ID is dropped, never shown.
  const IDENTITY_FIELDS = ['business_type', 'industry', 'official_address', 'website', 'udyam_number', 'other_businesses', 'key_people'];
  const presentIdFields = IDENTITY_FIELDS.filter((f) => Array.isArray(s[f]) ? _arr(s[f]).length > 0 : !!_s(s[f]));
  const allFlagged = presentIdFields.length > 0 && presentIdFields.every((f) => nskFields.has(f));
  const suppress = mc === 'none' || mc === 'low' || allFlagged;
  if (suppress) {
    emit('web_suppressed', `⚠ WEB RESULT SUPPRESSED — the web engine could not tie its findings to THIS buyer (${mc ? 'match_confidence=' + mc : 'every identity field is namesake-flagged'}). This is almost certainly a similarly-named DIFFERENT business (the classic namesake trap). NONE of the web business-type / address / website / scale / socials are surfaced, and NO attribute may be set from web here. Rely on the verified registry (GST / Udyam / PAN) instead.`);
    return true;
  }
  ...
  if (size) emit('web_scale', `Scale: ${size} [web — size / vintage signal]${cite('turnover_estimate') || cite('employee_count') || cite('year_established')}${nsk('turnover_estimate') || nsk('employee_count') || nsk('year_established')}`);
  if (_s(s.udyam_number)) emit('web_udyam', `Udyam/MSME (web): ${_s(s.udyam_number)} [web — verify vs Udyam registry]${cite('udyam_number')}${nsk('udyam_number')}`);
  const soc: string[] = [];
  ...
  if (soc.length) emit('web_digital', `Digital footprint: ${soc.join(', ')} [web — digital maturity / legitimacy]${cite('google_business')}${nsk('google_business') || nsk('linkedin') || nsk('facebook') || nsk('instagram') || nsk('twitter_x')}`);
  const others = _arr(s.other_businesses).map(_s).filter(Boolean); if (others.length) emit('web_other_biz', `Other businesses (web, UNVERIFIED — verify vs anchors): ${others.join('; ')}${cite('other_businesses')}${nsk('other_businesses')}`);
  const people = _arr(s.key_people).map(_s).filter(Boolean); if (people.length) emit('web_people', `Key people: ${people.join(', ')} [web]${cite('key_people')}${nsk('key_people')}`);
  const news = _arr(s.recent_news).map(_s).filter(Boolean); if (news.length) emit('web_news', `Recent activity: ${news.slice(0, 3).join(' | ')} [web]${cite('recent_news')}${nsk('recent_news')}`);
  return true;
}
...
export const EXTRACT_BUYER_PROFILE_SYSTEM = [
  ...
  "- WEIGHT SOURCES, DON'T COUNT THEM: weigh each source by its trust + authority; do NOT merely tally how many mention a thing. When evidence conflicts, resolve it PER the # SOURCE POLICIES block injected below — each attribute carries its OWN priority order there, and a recurring Requirement (repeated BuyLeads/ISQ) is a PRIMARY signal, not a low one. Do NOT apply a single fixed cross-source ranking here; the per-dimension frozen policies (and any live SOURCE GUIDE / CONFLICT PRIORITY) are authoritative.",
  ...
  "- buyer_maturity = where the business is in its life — say it in simple B2B terms: \"early-stage business\" (setting up a new unit / first venture / just installed machines) vs \"established business\". THREE-WAY, NEVER a default (audit fix — do not fabricate 'established'): (a) \"early-stage business\" ONLY when setup/new-venture signals appear (PNS seller-discovery: \"Are you setting up a unit or already working?\", \"Have you installed the machines?\", \"What did you do before this?\"); (b) \"established business\" ONLY with POSITIVE establishment evidence — a verified-business flag, GST/Udyam registration vintage (≥ ~2 yrs), multi-year account tenure, or repeat multi-year requirement history; (c) OTHERWISE OMIT the attribute entirely and add a needs_input entry — do NOT guess 'established'. (Displayed as \"Buyer Maturity\"; avoid jargon like \"New-entrant\".) Preferred sources: PNS seller-discovery / new-venture narratives (early-stage) > verified-business flag + GST/Udyam vintage + tenure (established).",
  "- business_stage = the life-stage as a dashboard token — \"Recently Established\" / \"Growing\" / \"Established\" (or \"early-stage business\" on setup signals). MUST agree with buyer_maturity, INCLUDING absence: if buyer_maturity is OMITTED for lack of evidence, OMIT business_stage too (never default to \"Established\"). Derive only from real setup signals or established-evidence. Preferred sources: PNS setup / new-venture signals > verified-business flag + account tenure (must agree with buyer_maturity).",
  ...
  "",
  "## PROCUREMENT",
  "- products_of_interest = the buyer's product lines. RANK by relevance to the buyer's CURRENT / core line (most recent + repeated categories first); DEMOTE clearly off-core one-offs (e.g. a stray vehicle / hardware lead for a paper buyer). Surface the top 3 most-relevant. If fewer than 3 are clearly on-core, fill with the next best, but mark them as the lower-relevance tail. Cite the evidence ids for each. INFER, DON'T ECHO (HOD): do NOT surface a raw colloquial / brand search term on its own — RESOLVE it to the real product category AND what it implies about the buyer's business. E.g. \"Chota Hathi\" → \"Commercial mini trucks (Tata Ace class) — signals a logistics / last-mile-delivery need, likely a distribution or transport expansion\"; \"CFL choke\" → \"Lighting electronics components — component-level sourcing for an assembler/manufacturer\". Keep the raw term in parentheses for traceability, but the VALUE must be the resolved category + the one-line business implication, never just the raw token. Preferred sources: PNS call sourcing-basket + recent & repeated BuyLeads / ISQ specs > CSL product / category views.",
