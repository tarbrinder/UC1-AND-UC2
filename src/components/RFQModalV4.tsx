...
import { fetchEnrichment, fetchCategoryIntel, fetchCategoryBuild, matchCategory, debugFallbackMobile, coreTokens, distillSessionBehavior, mergeObservedBehavior, getServerTrace, getEnrichmentRaw } from '../lib/enrichment';
...
import { buildInspectorPayload, globalSummary, type InspectorState } from '../lib/inspectorData';
import { observatoryReport } from '../lib/observatoryView';
import { allLineage } from '../lib/lineage';
import { buildSnapshot, saveRun, listRuns } from '../lib/replay';
import { startTraceExport } from '../lib/traceExport';
...
interface Props {
  ...
  stagingOnly?: boolean;
  onStart?: () => void;
  // P6 · LIVE RFQ LEDGER — emit the final inspectorState when this modal unmounts so MainApp's RFQ Ledger
  // reflects the REAL run (not the static demo). Loop-free: fired once on close from a ref, never per render.
...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function RFQModalV4({ onClose, variantLabel, initialGlid, autoPull, initialIgnoreTwin, stagingOnly, onStart, onInspectorState }: Props) {
  ...
  // ── V4 AI-INSPECTOR: which LLM-driven element is hovered (preview) / pinned (locked) on the left form.
  //    Mapped from the nearest [data-inspect="kind:key"] ancestor of the event target. Pure UI state.
  const leftPaneRef = useRef<HTMLDivElement>(null);
  const [inspectHover, setInspectHover] = useState<string | null>(null);
  const [inspectPin, setInspectPin] = useState<string | null>(null);
  const [inspectPin2, setInspectPin2] = useState<string | null>(null); // second pin → compare mode (A vs B)
  const inspectKeyFrom = (t: EventTarget | null): string | null => {
    let el = t as HTMLElement | null;
    while (el && el !== leftPaneRef.current) { const k = el.getAttribute && el.getAttribute('data-inspect'); if (k) return k; el = el.parentElement; }
    return null;
  };
  const onInspectHover = (e: { target: EventTarget | null }) => { const k = inspectKeyFrom(e.target); if (k) setInspectHover(k); };
  const onInspectClick = (e: { target: EventTarget | null }) => {
    const k = inspectKeyFrom(e.target); if (!k) return;
    if (inspectPin === k) { setInspectPin(inspectPin2); setInspectPin2(null); }   // unpin A → promote B to A
    else if (inspectPin2 === k) setInspectPin2(null);                              // unpin B
    else if (!inspectPin) setInspectPin(k);                                        // first click → pin A
    else setInspectPin2(k);                                                        // second distinct click → pin B (compare)
  };
  // Optional Langfuse export (env-gated → completely inert without VITE_LANGFUSE_* keys). Ships the LLM
  // records + eval runs we already capture, for cross-session history/search. Zero engine change; V3 never starts it.
  const traceSessionId = useRef('v4-' + Math.random().toString(36).slice(2, 10));
  useEffect(() => startTraceExport({
    getRecords: () => getLLMHealth().map((c) => ({ label: c.label, model: c.model, ms: c.ms, promptTokens: c.promptTokens, completionTokens: c.completionTokens, costUsd: c.costUsd, promptVersion: c.promptVersion, at: c.at, ok: c.ok })),
    getEvals: () => getEvalRuns().flatMap((r) => [{ kind: 'System', pct: r.systemPct, at: r.ts }, { kind: 'Business', pct: r.businessPct, at: r.ts }]),
    sessionId: traceSessionId.current,
    traceName: 'RFQ V4 · ' + (form.productName || 'session'),
  ...
  const handleGlidFetch = async (glidOverride?: string, ignoreTwinOverride?: boolean) => {
    ...
    if (profile) {
      ...
      //    Default OFF ⇒ byte-for-byte legacy. inspectorState.buyer reads the same state vars, so it follows. ──
  ...
  const renderSpecField = (spec: ISQSpec) => {
    ...
    return (
      <div key={spec.IM_SPEC_MASTER_DESC} data-inspect={(isSuggested ? 'autofilled-spec:' : 'spec:') + spec.IM_SPEC_MASTER_DESC} className="space-y-5">
        <div>
  ...
  const renderDynQuestion = (q: DynQuestion, onPick?: () => void) => {
    ...
    return (
      <div key={q.id} data-inspect={'planner-q:' + q.id} className="animate-field-in">
    ...
    <div>
      ...
      <div className="rounded-2xl border border-gray-100 p-4 sm:p-5 mb-4">
        ...
        <div className="flex flex-col sm:grid sm:grid-cols-2 gap-4 sm:gap-6">
          <div data-inspect="lastpage:deliveryTimeline">
          ...
          <div data-inspect="lastpage:paymentTerms">
            <p className="flex items-center gap-1.5 text-xs uppercase font-semibold text-gray-500 mb-2 tracking-wide">
  ...
  // Inherited dead code from the V3 baseline (unused there too — part of the 3 known baseline tsc notes).
  // Referenced so the V4 replica adds ZERO new tsc errors. Remove if/when wired into V4.
  void replanPending; void renderConciergeConfirm;

  // ── V4 AI-INSPECTOR: assemble the live decision state the inspector reads. PURE mapping of state the
  //    form already computes (no new LLM/compute) → src/lib/inspectorData.ts shapes the hovered payload. ──
  const nrm = (s: string) => String(s || '').toLowerCase().replace(/[^a-z0-9]/g, '');
  const inspectorState: InspectorState = (() => {
    const tw = (() => { try { return ignoreTwin ? null : liveTwin(); } catch { return null; } })();
    const twPlan = tw ? buildTwinPlanInput(tw, form.productName) : null;
    const specRows = isqSpecs.map((sp) => {
      const name = sp.IM_SPEC_MASTER_DESC;
      const val = form.dynamicSpecs[name] || '';
      const cov = coverage.current.coveredBy(name);
      const crit = (resolvedRequirement.criticalRanked || []).find((c) => nrm(c.maps_to_isq || '') === nrm(name) || nrm(c.name || '') === nrm(name));
      const order = (reqPlan?.specOrder || []).findIndex((s) => nrm(s) === nrm(name));
      return {
        name, value: val || undefined,
        source: cascadeSpecs.has(name) ? 'cascade-inferred' : val ? 'user' : (cov?.source || 'unknown'),
        status: val ? 'FILLED' : 'ASKED',
        priority: order >= 0 ? Math.max(40, 100 - order * 6) : undefined,
        sellerFreq: typeof crit?.seller_frequency === 'number' ? crit.seller_frequency : undefined,
        reason: reqPlan?.specReasons?.[name],
      };
    });
    const conf = {
      buyer: buyerTwin?.twin_confidence?.overall_score ?? undefined,
      category: categoryStatus === 'hit' ? categoryConf.score : undefined,
      intent: requirementIntent?.confidence || undefined,
      specs: isqSpecs.length ? Math.round((specRows.filter((s) => s.value).length / isqSpecs.length) * 100) : undefined,
      planner: reqPlan?.questions?.length ? Math.round((reqPlan.questions.filter((q) => q.groundedIn || q.reason).length / reqPlan.questions.length) * 100) : undefined,
    };
    const vals = [conf.buyer, conf.category, conf.intent, conf.specs, conf.planner].filter((x): x is number => typeof x === 'number');
    const overall = vals.length ? Math.round(vals.reduce((a, b) => a + b, 0) / vals.length) : undefined;
    // Tier-A human-override tracker — where the buyer changed an AI-deduced logistics value (pure compare).
    const FIELD_KEYS = ['paymentTerms', 'paymentMode', 'creditPeriod', 'deliveryTimeline', 'deliveryLocation'];
    const overrides = Object.entries(deducedLogistics || {}).flatMap(([k, d]) => {
      const cur = FIELD_KEYS.includes(k) ? String((form as unknown as Record<string, unknown>)[k] ?? '') : '';
      return d && d.value && cur && nrm(cur) !== nrm(d.value) ? [{ field: k, suggested: d.value, chosen: cur }] : [];
    });
    return {
      intent: requirementIntent ? {
        value: requirementIntent.value, journey: requirementIntent.journey, question: requirementIntent.question,
        confidence: requirementIntent.confidence, source: requirementIntent.source, locked: requirementIntent.locked,
        candidates: intentCandidatesRef.current,
        decision: intentDecisionRef.current ? { registry: intentDecisionRef.current.registry, twin: intentDecisionRef.current.twin, llm: intentDecisionRef.current.llm, chosenValue: intentDecisionRef.current.chosenValue, threshold: intentDecisionRef.current.threshold } : null,
      } : undefined,
      planner: reqPlan ? {
        archetype: reqPlan.archetype,
        questions: reqPlan.questions.map((q) => ({ id: q.id, label: q.label, tier: q.tier, priority: q.priority, reason: q.reason, groundedIn: q.groundedIn })),
        considered: reqPlan.considered, twinResolved: reqPlan.twinResolved, twinMode: reqPlan.twinMode, budgetUsed: reqPlan.questions.length, budgetMax: 3,
      } : undefined,
      category: {
        status: categoryStatus, score: categoryConf.score, band: categoryConf.band, consume: categoryConf.consume, fuse: categoryConf.fuse,
        criticals: resolvedRequirement.criticalRanked, blockers: (categoryConsumed.checks || []).map((c) => ({ label: c.label, frequency: c.frequency })), applications: categoryConsumed.applications,
      },
      specs: specRows,
      logistics: deducedLogistics,
      buyer: { persona: buyerProfile?.persona, nature: buyerProfile?.nature, authority: buyerProfile?.authority, maturity: buyerProfile?.maturity, twinConfidence: buyerTwin?.twin_confidence?.overall_score, verified: !!buyerTwin?.layer_a_identity?.verified, evidenceCount: twPlan?.whyKnown?.length, offProfile: twPlan?.offProfile,
        apiPersona: enrichment?.persona ? { type: enrichment.persona.type, scale: enrichment.persona.scale, repeatBuyer: enrichment.persona.repeatBuyer, multiSku: enrichment.persona.multiSku, whatsappAffinity: enrichment.persona.whatsappAffinity } : undefined },
      llmCalls: getLLMHealth().map((c) => ({ label: c.label, ms: c.ms, model: c.model, promptVersion: c.promptVersion, promptTokens: c.promptTokens, completionTokens: c.completionTokens, costUsd: c.costUsd })),
      confidence: { ...conf, overall },
      overrides,
      // per-element deep detail: raw prompt I/O (every LLM label) + fact lineage (buyer facts → JSON field → n8n node)
      llmRaw: getLLMRaw(),
      lineage: allLineage(getEnrichmentRaw(), getServerTrace()),
    };
  })();
  // P5 · snapshot each settled run for Replay (debounced by a content signature so we save once per run)
  const savedRunSig = useRef('');
  const runSig = `${requirementIntent?.value || ''}|${reqPlan?.questions?.length ?? 0}|${categoryConf?.score ?? 0}|${form.productName}`;
  useEffect(() => {
    if (!(requirementIntent?.value || reqPlan)) return; // not settled enough to be a meaningful run
    if (savedRunSig.current === runSig) return;
    savedRunSig.current = runSig;
    try { saveRun(buildSnapshot(inspectorState, { id: `${traceSessionId.current}:${Date.now()}`, at: Date.now(), label: form.productName || 'run' })); } catch { /* noop */ }
  }, [runSig]); // eslint-disable-line react-hooks/exhaustive-deps

  const inspPayload = buildInspectorPayload(inspectPin ?? inspectHover, inspectorState);
  const inspPayloadB = buildInspectorPayload(inspectPin2, inspectorState); // compare slot
  const inspSummary = globalSummary(inspectorState);
  // P6 · keep the latest inspectorState + callback in refs, then emit ONCE on unmount (form close). No effect
  // deps on inspectorState (a fresh object each render) ⇒ zero render-loop risk; MainApp gets the final run.
  const inspStateRef = useRef(inspectorState); inspStateRef.current = inspectorState;
  const onInspectorStateRef = useRef(onInspectorState); onInspectorStateRef.current = onInspectorState;
  useEffect(() => () => { try { onInspectorStateRef.current?.(inspStateRef.current); } catch { /* noop */ } }, []);
  // 🛰 OBSERVATORY lens — runs observatory.ts (L11-L20) + qualityGates.ts (bl_quality) on the live state
  //    + raw form fields, so the two libs are CONSUMED (visible), not dead code. PURE, no new LLM/compute.
  const inspObservatory = observatoryReport({
    state: inspectorState,
    productName: form.productName,
    quantity: Number(form.quantity) || 0,
    unit: form.unit,
    freeText: `${form.requirementNotes || ''} ${form.voiceTranscript || ''}`.trim(),
    gstOnFile: form.gstRegistered === true,
    companyOnFile: !!(form.additionalDetails || '').trim(),
    serverTrace: getServerTrace(), // n8n E1 `_trace` from the buyer pull (null when E1 inactive → "not active")
    raw: getEnrichmentRaw(),       // buyer-pull raw JSON → fact→JSON-path→node lineage (P1)
    llmRaw: getLLMRaw(),           // raw prompt I/O per call → output-acceptance ledger (P2)
    runs: listRuns(),              // persisted run snapshots → Replay A↔B diff (P5)
    twin: buyerTwin,               // BuyerTwin → brain-construction (W1)
    requirement: resolvedRequirement,  // ResolvedRequirement → active/superseded facts (W1)
    contradictions: { count: contradictions.length, items: contradictions.map((n) => n.question || n.type) }, // W1 · Q73/74
    specOrder: reqPlan?.specOrder || resolvedRequirement?.specOrder || [],  // spec-reasoning ranks (W2)
    outcome: null,                 // no real lead/call-outcome feed yet → W7 shows "awaiting feed" (honest)
    // mcatMedianPrice / mcatQ1-Q3 / productPrices: not wired to the form yet → POV + price-as-qty rule
    // degrade gracefully (flagged "no category median price wired (E2)") rather than fabricate a number.
  });

  // ─── Render (V4 — 50/50 split: mobile-framed form on the left, AI Inspector on the right) ──────
  return (
    <div className="fixed inset-0 z-50 flex bg-gray-200">
    {/* LEFT — the real RFQ form (mobile-framed). Hover/click any AI-driven element → inspect on the right. */}
    <div
      ref={leftPaneRef}
      onMouseOver={onInspectHover}
      onMouseLeave={() => setInspectHover(null)}
      onClick={onInspectClick}
      className="w-1/2 h-full relative bg-black/40 backdrop-blur-sm overflow-y-auto"
        ...
        {stagingOnly ? renderStagingView() : step === 0 ? (
          ...
          <div className="flex h-full" style={{ minHeight: 520 }}>
            ...
            <div className="flex-1 flex flex-col p-6 min-w-0 relative">
              ...
              {renderBuyerKindNote()}
              <div data-inspect="intent">{renderPage1Intent()}</div>
          ...
          <div className="flex flex-col max-h-[88vh]">
            ...
            <div ref={scrollRef} className="flex-1 min-h-0 overflow-y-auto scroll-auto-hide px-5 py-5 space-y-5">
              ...
              {step === 2 && <div data-inspect="profile">{renderDeliveryPage()}</div>}
            </div>
    ...
    {/* RIGHT — AI Inspector (hover = preview · click = pin) */}
    <aside className="w-1/2 h-full border-l border-gray-300 bg-white overflow-hidden">
      <AIInspector payload={inspPayload} payloadB={inspPayloadB} summary={inspSummary} observatory={inspObservatory} state={inspectorState} pinned={!!inspectPin} onUnpin={() => setInspectPin(null)} onUnpinB={() => setInspectPin2(null)} />
    </aside>
    </div>
  );
}
