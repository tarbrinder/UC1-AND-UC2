...
// Open with ?profile=<GLID> → this self-fetches the INDEPENDENT bi-buyer-profile endpoint (v16.4 pipeline + server-side
...
export default function BuyerProfileStandalone({ glid }: { glid: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      let data: unknown = null; let via = 'bi-buyer-profile';
      ...
      if (!data) {
        via = 'main pull (fallback — bi-buyer-profile not reachable)';
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 text-gray-600">
        <div className="text-center">
          ...
          <div className="text-[11px] text-gray-400 mt-1">Trying the dedicated bi-buyer-profile endpoint, then falling back to the live main pull. Can take a few minutes.</div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 text-rose-600">
        <div className="text-center max-w-md">
          ...
          <div className="text-[11px] text-gray-500 mt-1.5">Both the <code>bi-buyer-profile</code> endpoint AND the main pull returned nothing — likely the dev server / n8n isn't reachable, or the GLID has no data. (Import the <code>bi-buyer-profile</code> workflow to enable the server-LLM path; until then this uses the main endpoint.)</div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-6xl mx-auto">
        {state.via.startsWith('main') && <div className="mb-2 text-[10px] text-amber-700 bg-amber-50 border border-amber-200 rounded px-2 py-1">ℹ Rendered via the main endpoint (the dedicated <code>bi-buyer-profile</code> workflow isn't imported/reachable) — GST/Udyam/business-type still derive; the server-LLM narrative overlay is absent.</div>}
