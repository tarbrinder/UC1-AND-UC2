...
const Icon = ({ ch }: { ch: string }) => <span className="w-4 shrink-0 text-gray-400 text-[12px] leading-none select-none">{ch}</span>;

function KV({ icon, label, f, link }: { icon: string; label: string; f: Field; link?: boolean }) {
  return (
...
function StatTile({ label, value, note, tone }: { label: string; value: number | null; note: string; tone: string }) {
  return (
    <div className="flex-1 px-4 py-2 text-center" title={note}>
      <div className={`mx-auto mb-1 w-8 h-8 rounded-full flex items-center justify-center ${tone}`}>
        <span className="w-2 h-2 rounded-full bg-white/90" />
      </div>
      <div className="text-2xl font-bold text-white leading-none tabular-nums">{value == null ? '—' : value}</div>
...
export default function BuyerProfileCard({ rich, glid, pending }: { rich: unknown; glid: string; pending?: boolean }) {
  ...
  const tileTones = ['bg-blue-900/40', 'bg-emerald-500/50', 'bg-violet-500/50'];
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white overflow-hidden shadow-sm">
      ...
      {/* header band */}
      <div className="bg-gradient-to-r from-[#0b1f4d] to-[#132c63] px-4 py-2.5 flex items-center gap-4">
        <div className="flex items-center gap-2 shrink-0">
          <span className="w-6 h-6 rounded-full bg-amber-400 text-[#0b1f4d] text-[11px] font-black flex items-center justify-center">S</span>
          <span className="text-white font-bold text-[13px] tracking-wide">TrustSEAL <span className="text-amber-400">Buyer Profile</span></span>
        </div>
        <div className="flex-1" />
        <div className="hidden sm:block text-right">
          <div className="text-[9px] text-white/60 uppercase tracking-wide mb-0.5 mr-2">Overall Activity (Last 6 Months)</div>
          <div className="flex items-stretch divide-x divide-white/15">
            {m.header.tiles.map((t, i) => <StatTile key={t.label} label={t.label} value={t.value} note={t.sourceNote} tone={tileTones[i] || 'bg-white/20'} />)}
          </div>
        </div>
      </div>
      ...
      <div className="px-4 pt-3 pb-2 border-b border-gray-100">
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1 text-[12px] text-gray-600">
          <span className="inline-flex items-center gap-1"><Icon ch="👤" />{m.header.contactName.present ? m.header.contactName.value : <span className="text-gray-300 italic">—</span>}</span>
          <span className="text-gray-300">|</span>
          <span className="inline-flex items-center gap-1"><Icon ch="📅" />Member Since: <b className="text-gray-700">{tenure != null ? `${tenure} Year${tenure === 1 ? '' : 's'}` : (m.header.memberSince.present ? m.header.memberSince.value : '—')}</b></span>
          <span className="text-gray-300">|</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[30%_40%_30%] gap-x-5 gap-y-3 p-4">
        ...
        <div>
          ...
          {m.overview.map((o) => <KV key={o.label} icon="◦" label={o.label} f={o.field} />)}
          <SectionTitle>Procurement Profile</SectionTitle>
          {m.procurement.map((o) => <KV key={o.label} icon="◦" label={o.label} f={o.field} />)}
          <SectionTitle>Market Focus</SectionTitle>
          {m.market.map((o) => <KV key={o.label} icon="◦" label={o.label} f={o.field} />)}
        </div>

        {/* ── COLUMN 2 — COMPANY DETAILS ── */}
        <div>
          <ColTitle>Company Details</ColTitle>
          <KV icon="₹" label="GST" f={m.company.gst} />
          <div className="flex items-start gap-2 py-1">
            <Icon ch="✓" />
          ...
          <KV icon="👤" label="Trade Name / Company Name" f={m.company.tradeName} />
          <KV icon="🏢" label="Constitution of Business" f={m.company.constitution} />
          <KV icon="📅" label="Date of Registration" f={m.company.regDate} />
          <KV icon="📍" label="Principal Place of Business" f={m.company.principalAddress} />
        ...
        {/* ── COLUMN 3 — SOCIAL / PRODUCTS ── */}
        <div>
          <ColTitle>Social Media Presence</ColTitle>
          <KV icon="🌐" label="Website" f={m.social.website} link />
          <KV icon="f" label="Facebook" f={m.social.facebook} link />
          <KV icon="◎" label="Instagram" f={m.social.instagram} link />
          <KV icon="in" label="LinkedIn" f={m.social.linkedin} link />
          <KV icon="✕" label="Twitter / X" f={m.social.twitter} link />
          {m.googleBusiness?.exists && m.googleBusiness.rating && (
            <KV icon="★" label="Google Business" f={{ value: m.googleBusiness.rating, present: true, provenance: 'inferred', source: 'web_osint', inferred: true }} />
          )}
