  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-gray-50 text-[13px] text-gray-700">
      ...
      {ledger && tab === 'ledger' && (() => {
        ...
        // V11 — LLM-derived (>1 source) profile attributes get the LLM badge; deterministic single-source facts are added below.
        const PROFILE_KEYS = ['business_persona', 'buyer_maturity', 'sub_industry', 'buyer_intent', 'scale', 'price_vs_quality', 'procurement_model', 'purchase_frequency', 'communication', 'delivery_timeline', 'identity_confidence', 'digital_footprint'];
        const llmRows: L6ProfileRow[] = PROFILE_KEYS.map((k) => finals.find((f) => f.key === k)).filter((f): f is NonNullable<typeof f> => !!f).map((f) => ({ label: f.label, value: `${f.value} · ${f.confidence}% · LLM`, drill: finalAttrDetail(f) }));
        // V11 — DETERMINISTIC (single-source passthrough) rows: name (full, with confidence band) · PAN entity · GST · age/gender · purchasing power · digital presence. No LLM reasoning → "deterministic" badge.
        const resolvedName = resolveBuyerName(idn, ext);
        // Wire the n8n Mobile/Email→GST result (sources.gst / derived_anchors.gst) into the decode when identity lacks a GST.
        ...
        if (resolvedName) detRows.push({ label: 'Name', value: `${resolvedName.name} · ${resolvedName.confidence}% · deterministic`, drill: (<div className="text-[10.5px] text-gray-600">Resolved from {resolvedName.source} (most-complete of Profile ⊕ verified). Confidence band {resolvedName.confidence}%{resolvedName.full ? ' · full name' : ' · first name only'}. PII — kept for the debug/requirement form.</div>) });
        if (ext?.age || ext?.gender) detRows.push({ label: 'Age / Gender', value: `${[ext?.age && `${ext.age}y`, ext?.gender].filter(Boolean).join(' · ')} · deterministic (Befisc)` });
        if (docs.pan) detRows.push({ label: 'PAN entity', value: `${docs.pan.entityType}${docs.pan.nameMatch !== 'unknown' ? ` · surname ${docs.pan.nameMatch}` : ''}${docs.panDuplicate ? ' · ⚠ 2 distinct PANs' : ''} · deterministic`, drill: (<div className="text-[10.5px] text-gray-600">PAN <span className="font-mono">{docs.pan.pan}</span>: 4th char "{docs.pan.entityChar}" → {docs.pan.entityType}; 5th char "{docs.pan.surnameInitial}" = surname initial ({docs.pan.nameMatch}). Format {docs.pan.valid ? 'valid' : 'invalid'}.</div>) });
        if (docs.gst) detRows.push({ label: 'GST', value: `${docs.gst.state} · ${docs.gst.entityType} · registered · deterministic`, drill: (<div className="text-[10.5px] text-gray-600">GSTIN <span className="font-mono">{docs.gst.gstin}</span>: state code {docs.gst.stateCode} → {docs.gst.state}; embedded PAN {docs.gst.pan} → {docs.gst.entityType}. Format {docs.gst.valid ? 'valid' : 'invalid'}.</div>) });
        if (ext?.incomeBand) detRows.push({ label: 'Purchasing power', value: `${ext.incomeBand} · deterministic (Befisc income — single source)` });
        if (ext?.socialPlatforms?.length) detRows.push({ label: 'Digital presence', value: `${ext.socialPlatforms.slice(0, 6).join(', ')}${ext.socialPresenceCount ? ` (${ext.socialPresenceCount})` : ''} · presence only`, drill: (<div className="text-[10.5px] text-gray-600">Social-platform presence (Sign3). Per-platform tenure ("since N years") is NOT in current data — a data-gap (candidate for the crawler / a future API). IndiaMART member-since shown above when present.</div>) });
        const profileRows: L6ProfileRow[] = [...detRows, ...llmRows];
        const buyerDetails = (idn || profileRows.length) ? { name: resolvedName?.name || idn?.name, memberSince: humanizeSince(idn?.memberSince), responseCalls: pnsCards.length, responseReplies: waConvo?.inbound.buyerMsgs ?? 0, availability, profileRows } : null;
        ...
        return (
          <div className="flex-1 min-h-0 overflow-y-auto p-4 bg-gray-50/40">
            <div className="max-w-3xl mx-auto space-y-2.5">
              ...
              <UC2DebugBand status={uc2LLM.status} model="google/gemini-2.5-flash" promptVersion="uc2Enrich.v2" usage={uc2Usage} evalRes={uc2Result?.eval ?? null} edits={uc2Result?.edits} input={uc2PromptIO} rawOutput={getLLMRaw()['uc2Enrich']?.output} coverage={pnsCoverage} />
              <L6Band picker={offerPicker} selectedReq={selectedReqCard} uc2={uc2} productsOfInterest={productsOfInterest} requirementCount={requirements.length} buyerDetails={buyerDetails} retailLead={selReq?.retailLead} titleDrill={titleDrill} locationDrill={locationDrill} fields={offerFields} offerEval={offerEvalCard} enrichControl={enrichControl} defaultOpen />
